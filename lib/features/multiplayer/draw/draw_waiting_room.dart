...
class DrawWaitingRoom extends StatefulWidget {
  final String roomCode;
  final bool isCreator;
  final int maxPlayers;

  const DrawWaitingRoom({
    super.key,
  ...
  @override
  State<DrawWaitingRoom> createState() => _DrawWaitingRoomState();
}

class _DrawWaitingRoomState extends State<DrawWaitingRoom> with SingleTickerProviderStateMixin {
  ...
  Future<void> _initAndSubscribe() async {
    ...
    try {
      if (widget.isCreator) {
        await client.from('draw_rooms').insert({
        ...
        // Read current players and update
  ...
  void _navigateToGame() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (!mounted) return;
      context.go('/draw/${widget.roomCode}?creator=${widget.isCreator}&players=${widget.maxPlayers}');
    });
  }
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              'JOINED PLAYERS (${_joinedPlayers.length}/${widget.maxPlayers})',
              style: GoogleFonts.rajdhani(
                fontWeight: FontWeight.bold,
                letterSpacing: 2,
                fontSize: 12,
            ...
            if (widget.isCreator) ...[
              ...
              if (!canStart) ...[
                const SizedBox(height: 12),
                Text(
                  'Waiting for at least 1 guest to connect...',
                  textAlign: TextAlign.center,
            ...
            if (widget.isCreator)
              FriendInvitePanel(roomCode: widget.roomCode, gameType: 'draw'),
          ],
        ),
      ),
    );
