...
import 'math_blaster_game.dart';

class MathBlasterScreen extends StatefulWidget {
  const MathBlasterScreen({super.key});

  @override
  State<MathBlasterScreen> createState() => _MathBlasterScreenState();
}

class _MathBlasterScreenState extends State<MathBlasterScreen> {
  MathBlasterGame? _game;
  ...
  Future<void> _initGame() async {
    final hs = await AchievementService.instance.getHighScore('math_blaster');
    if (!mounted) return;

    final game = MathBlasterGame(
      bestScore: hs,
      onStateUpdate: (score, lives, level) {
        if (mounted) {
          setState(() {
            _score = score;
            _lives = lives;
      ...
      onGameOver: (score, isNewRecord) {
        if (mounted) {
          ...
          AchievementService.instance.saveHighScore('math_blaster', score);
              ...
              children: [
                ...
                Row(
                  ...
                  children: [
                    ...
                    Text('$_lives', style: GoogleFonts.pressStart2p(fontSize: 10, color: const Color(0xFFF05A28))),
                  ],
                ),
              ],
            ),
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              Text('MATH BLASTER', style: GoogleFonts.pressStart2p(fontSize: 16, color: const Color(0xFFF05A28), fontWeight: FontWeight.bold)),
              const SizedBox(height: 6),
              Text('POP CORRECT EQUATIONS', style: GoogleFonts.shareTechMono(fontSize: 14, color: AppColors.textSecondary, letterSpacing: 3)),
              ...
              SizedBox(
                ...
                child: ElevatedButton(
                  ...
                  child: Text('START BLASTER', style: GoogleFonts.pressStart2p(fontSize: 10, color: Colors.white, fontWeight: FontWeight.bold)),
                ),
              ),
            ],
          ),
