...
class _ColorRushScreenState extends State<ColorRushScreen> {
  ...
  Future<void> _initGame() async {
    final hs = await AchievementService.instance.getHighScore('color_rush');
    if (!mounted) return;

    final game = ColorRushGame(
      bestScore: hs,
      onStateUpdate: (score, combo, best) {
        if (mounted) {
          setState(() {
            _score = score;
            _combo = combo;
      ...
      onGameOver: (score, isNewRecord) {
        if (mounted) {
          ...
          AchievementService.instance.saveHighScore('color_rush', score);
        ...
        children: [
          ...
          Expanded(
            child: Column(
              ...
              children: [
                const SizedBox(height: 32),
                Text('COMBO', style: GoogleFonts.pressStart2p(fontSize: 6, color: AppColors.textSecondary)),
                const SizedBox(height: 2),
                Text('x$_combo', style: GoogleFonts.pressStart2p(fontSize: 12, color: const Color(0xFFF05A28))),
              ],
            ),
          ),
        ],
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              Text('COLOR RUSH', style: GoogleFonts.pressStart2p(fontSize: 16, color: const Color(0xFFF05A28), fontWeight: FontWeight.bold)),
              const SizedBox(height: 6),
              Text('TAP L/R TO ROTATE', style: GoogleFonts.shareTechMono(fontSize: 14, color: AppColors.textSecondary, letterSpacing: 3)),
              ...
              SizedBox(
                ...
                child: ElevatedButton(
                  ...
                  child: Text('START RUSH', style: GoogleFonts.pressStart2p(fontSize: 10, color: Colors.white, fontWeight: FontWeight.bold)),
                ),
              ),
            ],
          ),
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              Text('WRONG COLOR', style: GoogleFonts.pressStart2p(fontSize: 16, color: AppColors.error, fontWeight: FontWeight.bold)),
